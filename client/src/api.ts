...
export type Tenant = { id: number; unit_id: number; name: string; email: string; monthly_prepayment_cents: number; portal_token: string };
...
export type Line = { invoice_id: number; provider: string; category: string; description: string | null; allocation_key: string; total_cents: number; basis_unit: number; basis_total: number; share_cents: number };
...
export const api = {
  ...
  statements: (pid: number, year: number) => req<Statement[]>(`/api/properties/${pid}/statements?year=${year}`),
  generate: (pid: number, year: number) => req<Statement[]>(`/api/properties/${pid}/statements/generate`, json({ year })),
